...
var (
	...
	status2xx int64
	status3xx int64
	status4xx int64 // Теперь включает в себя и 418 Teapot
	status5xx int64

	// Счетчики сетевых ошибок (клиент не получит HTTP статус)
	timeoutErrors int64 // Клиент получит 2xx, если дождется, или ошибку таймаута
	resetErrors   int64 // Соединение сброшено (RST)
	eofErrors     int64 // Соединение закрыто (EOF)

	// Счетчик успешно обработанных HTTP запросов (дошедших до ответа)
	totalHandled int64
)
...
func resetCounters() {
	...
	atomic.StoreInt64(&totalHandled, 0)
...
func printStats() {
	...
	for range ticker.C {
		...
		handled := atomic.LoadInt64(&totalHandled)
		...
		// Общее количество попыток подключения (совпадает с суммой запросов у Bombardier)
		totalIncoming := handled + rErrors + eErrors

		fmt.Println("\n" + strings.Repeat("=", 60))
		fmt.Println("СТАТИСТИКА СЕРВЕРА (обновление каждые 5 сек)")
		fmt.Println(strings.Repeat("=", 60))

		fmt.Printf("Всего входящих соединений: %d\n", totalIncoming)
		fmt.Printf("Успешно отправлено HTTP ответов: %d\n", handled)
		...
		fmt.Println("\nСЕТЕВЫЕ ОШИБКИ:")
		fmt.Printf("  Connection Reset (RST): %d\n", rErrors)
		fmt.Printf("  Premature EOF: %d\n", eErrors)
		fmt.Printf("  Искусственные задержки (Timeouts): %d\n", tErrors)

		if totalIncoming > 0 {
			totalFloat := float64(totalIncoming)
			fmt.Println("\nПРОЦЕНТНОЕ СООТНОШЕНИЕ (от всех соединений):")
			fmt.Printf("  2xx (всего): %.2f%%\n", float64(s2xx)/totalFloat*100)
			fmt.Printf("  3xx: %.2f%%\n", float64(s3xx)/totalFloat*100)
...
func main() {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Примечание: не инкрементируем totalHandled, так как HTTP ответ не формируется
		case randomNum < 4:
			incrementCounter(&resetErrors)
			// fmt.Printf("[RESET] %s - Closing with RST packet\n", r.RemoteAddr) // Закомментировано для высоких RPS
			...
			// Устанавливаем SO_LINGER с таймаутом 0, чтобы вызвать жесткий RST
		...
		// Примечание: не инкрементируем totalHandled
		case randomNum < 8:
		...
		// Таймаут (имитация медленной работы) - 6%
		// Клиент получит 200 OK, если его собственный таймаут не сработает раньше
		case randomNum < 14:
			incrementCounter(&totalHandled)
		...
		// 418 I'm a teapot (раньше был Other, теперь считается как 4xx) - 5%
		case randomNum < 21:
			incrementCounter(&totalHandled)
	...
	fmt.Println("  4% - Connection Reset (RST packet) -> Клиент получит ошибку сокета")
	fmt.Println("  4% - Premature EOF -> Клиент получит ошибку сокета")
	fmt.Println("  6% - Таймаут (2 сек, потом 200 OK)")
	fmt.Println("  2% - 204 No Content")
	fmt.Println("  5% - 418 Teapot")
	fmt.Println("  6% - 3xx")
	fmt.Println("  8% - 5xx")
	fmt.Println("  10% - 4xx")
	fmt.Println("  55% - 200 OK")
	fmt.Println("\nСумма 2xx: ~63% | Сумма 4xx: ~15%")
	fmt.Println("\nСтатистика выводится каждые 5 сек")
	fmt.Println("Введите 'reset' для сброса счетчиков\n")
